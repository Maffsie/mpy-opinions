...
class BreakoutQwiicAlNum(I2CPeripheral):
    """
    ...
    def init(self, brightness: int = None, auto_write: bool = False):
        if brightness is not None: self.default_brightness = brightness
    ...
    def blank(self, write: bool = False):
        ...
        if self.auto_write or write: self.show()
    ...
    def put(self, data: str, offset: int = 0, write: bool = False, **kwargs):
        ...
        if self.auto_write or write: self.show(**kwargs)
    ...
    def put_one(self, data, offset: int = 0, write: bool = False, **kwargs):
        ...
        def resolve(segment, offset):
            ...
            com = segment
            if segment > 8: com -= 7
            elif segment == 8: com = 0
            elif segment == 7: com = 1
    ...
    def show(self, **kwargs):
        ...
        """
        if 'with_colon' in kwargs.keys(): self.colon = kwargs['with_colon']
        if 'with_dot' in kwargs.keys(): self.dot = kwargs['with_dot']
    ...
    def colon(self, state: bool):
        if state:
            ...
            self.buf[self.sym[':']] &= ~self.state['on']
        if self.auto_write or type(state) is int: self.show()
